	...
	for rows := 0; rows < len(array040100); rows++ {
		for column := 0; column < len(array040100); column++ {
			...
			if rows%4 == 0 {
				...
				array040100[rows][n-1-column] = int32(num) // if you only write n-1, only 1 column (n-1 = 8) will be affected
				num += 1                                   // but if you write [n-1-column] another column will be affected, because "column" variable used as loop variable
			} else if rows%4 == 3 && column == n-n {
				array040100[rows][column] = int32(num)
				num += 1
			}
		}
...
// when analysing printing pattern, always look for star-end coordinate point
// when analysing number pattern, always look for the base-case number sequences
// when analysing array pattern, always look for the length of every side

func Logic04Soal01arr01(n int) {
}
